...
def create_test_data(CONTEXT_LENGTH, PREDICTION_HORIZON, IN_DIM, OUT_DIM, split, device):
    try:
        raw_data = np.load("./data/train_data_lorenz_full.npy")
...
checkpoint_path = "./code/checkpoints/lorenz_8_finite-diff-w_SIMPLE_1_1024_2_QRNN_1385_LAST.pth"
...
for epoch in range(0,EPOCHS):
    ...
    for batch_idx, (input_seq, target_seq) in tqdm(enumerate(train_loader), total=len(train_loader), desc=f"Epoch {epoch+1}"):
        ...
        if i % 100 == 0:
            torch.save(model.state_dict(), f'./code/checkpoints/latest_spsa/lorenz_{N_QUBITS}_{DIFF_METHOD}_SIMPLE_{REPEAT_BLOCKS}_{SHOTS}_{epoch+1}_QRNN_{i}.pth')
    ...
    last_checkpoint_path = f'./code/checkpoints/latest_spsa/lorenz_{N_QUBITS}_{DIFF_METHOD}_SIMPLE_{REPEAT_BLOCKS}_{SHOTS}_{epoch+1}_QRNN_{i}_LAST.pth'
